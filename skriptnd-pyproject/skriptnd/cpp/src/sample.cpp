...
{
    auto error_handler = [&]( const ts::Position& position, const std::string& message, const ts::StackTrace& stacktrace, const bool warning )
    {
    ...
    bool all = false;
    bool verbose = false;
    ts::OperationCallback atomic = ts::FalseOperationCallback;
    ts::OperationCallback unroll = ts::FalseOperationCallback;
    for ( size_t i = 2; i < argc; ++i )
    {
        ...
        {
            atomic = ts::TrueOperationCallback;
        }
        else if ( arg == "--unroll" )
        {
            unroll = ts::TrueOperationCallback;
        }
    }
    ...
    auto graph_names = ts::enum_graph_names(is);
