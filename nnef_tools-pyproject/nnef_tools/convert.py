...
def get_reader(input_format, atomic, decomposed, fold_constants, custom_shapes):
    if input_format == 'sknd':
        ...
        return Reader(atomic=lambda op: op.name in atomic, inline=lambda op: op.name in decomposed)
            ...
            if optimizer:
                if args.output_format != args.input_format:
                    ...
                    reader = get_reader(args.output_format, atomic=lambda name: not name.startswith('main.'),
                                        decomposed=decomposed, custom_shapes=custom_shapes, fold_constants=False)
