...
class ExprConverter:
    ...
    def __call__(self, expr: nd.Expr):
        return self._expr_root(expr)

    def get_call_indices(self, access: nd.TensorAccess):
    ...
    def _expr_root(self, expr: nd.Expr):
        if not isinstance(expr, nd.Expr):
        ...
        return {
            nd.SizeAccess: self._size_access,
            nd.ShapeAccess: self._shape_access,
            nd.TensorAccess: self._tensor_access,
            nd.IdentifierExpr: self._identifier_expr,
            nd.ReferenceExpr: self._reference_expr,
            nd.UnaryExpr: self._unary_expr,
            nd.BinaryExpr: self._binary_expr,
            nd.SelectExpr: self._select_expr,
            nd.FoldExpr: self._fold_expr,
            nd.ListExpr: self._list_exprs,
            nd.CastExpr: self._cast_expr,
            nd.BoundedExpr: self._bounded_expr,
            nd.ConcatExpr: self._concat_expr,
            nd.SliceExpr: self._slice_expr,
            nd.SubscriptExpr: self._subscript_expr,
            nd.UniformExpr: self._uniform_expr,
            nd.RangeExpr: self._range_expr,
        }[type(expr)](expr)

    def _size_access(self, expr: nd.SizeAccess):
        obj = expr.pack
        return len(obj)

    def _shape_access(self, expr: nd.ShapeAccess):
        if self.ns == te:
    ...
    def _tensor_access(self, expr: nd.TensorAccess):
        # assert self.env.indices is not None
        if isinstance(expr.tensor, nd.Tensor):
            # tens = self.env.instance_tensors[expr.tensor.name]
            tens = self.env[expr.tensor.name]
        elif isinstance(expr.tensor, nd.TensorPack):
    ...
    def _identifier_expr(self, expr: nd.IdentifierExpr):
        elem = self.env[expr.name]
        if isinstance(elem, tir.IterVar):
            return elem.var
        return elem

    def _reference_expr(self, expr: nd.ReferenceExpr):
        raise ValueError("ReferenceExpr not implemented")

    def _unary_expr(self, expr: nd.UnaryExpr):
        op_dict = {
            "+": lambda x: x,
            "-": lambda x: -x,
    ...
    def _binary_expr(self, expr: nd.BinaryExpr):
        dtype = expr.dtype
        op_dict = {
            "+": self.ns.add,
            "-": self.ns.subtract,
            "*": self.ns.multiply,
            "/": lambda x, y: x / y,
            "\\": lambda x, y: (x + y - 1) / y if dtype == nd.Dtype.Int else self.ns.ceil(x / y),
            "**": self.ns.power,
            "<": lambda x, y: x < y,
            "<=": lambda x, y: x <= y,
            ">": lambda x, y: x > y,
            ...
            "%": (tir.fmod if dtype == nd.Dtype.Real else lambda x, y: x % y) if self.ns == te else relax.op.mod,
    ...
    def _select_expr(self, expr: nd.SelectExpr):
    ...
    def _concat_expr(self, expr: nd.ConcatExpr):
        raise ValueError("ConcatExpr not implemented")

    def _slice_expr(self, expr: nd.SliceExpr):
        raise ValueError("SliceExpr not implemented")

    def _subscript_expr(self, expr: nd.SubscriptExpr):
    ...
    def _uniform_expr(self, expr: nd.UniformExpr):
        ...
        dtype = convert_dtype(expr.dtype)
        if isinstance(expr.value, nd.Expr):
    ...
    def _range_expr(self, expr: nd.RangeExpr):
