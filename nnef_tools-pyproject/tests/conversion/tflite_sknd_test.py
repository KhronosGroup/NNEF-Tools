...
class TestEnv(sknd_test.TestEnv):
    ...
    def _convert_to_sknd(self, filename, input_shape=None):
        tflite_model = self._tflite_reader(filename)
        if self._optimize:
            self._tflite_optimizer(tflite_model)

        sknd_model = self._tflite_to_sknd_converter(tflite_model)
    ...
    @staticmethod
    def _save_default_graph(inputs, outputs, filename):
    ...
    def _test_conversion(self, name, inputs, outputs, epsilon=1e-4, input_range=None):
...
class TestCases(TestEnv):
    ...
    def test_batch_to_space(self):
        ...
        self._test_conversion('batch_to_space', [input], [output])

    def test_space_to_batch(self):
        input = tf.placeholder(shape=(16, 32, 32, 128), dtype=tf.float32)
        output = tf.space_to_batch(input, [[0, 0], [0, 0]], 2)

        self._test_conversion('space_to_batch', [input], [output])

    def test_depth_to_space(self):
        input = tf.placeholder(shape=(16, 32, 32, 128), dtype=tf.float32)
        output = tf.nn.depth_to_space(input, 2)

        self._test_conversion('depth_to_space', [input], [output])

    def test_space_to_depth(self):
        input = tf.placeholder(shape=(16, 32, 32, 128), dtype=tf.float32)
        output = tf.nn.space_to_depth(input, 2)

        self._test_conversion('space_to_depth', [input], [output])
